...
void GameXML::buildXMLScene(std::string requested_level_name, GameManager* game_man)
{
	...
	while(level_element != NULL)
	{
		...
		if(requested_level_name == level_name_str)
		{
			...
			game_manager->createCamera(camera_name_str, camera_position, camera_lookat, );
			...
			float light_color[3];
			Utils::parseFloats(light_color_str, loight_color);
...
void GameXML::processChildren(TiXmlElement* children_element, std::string parent_name_s)
{
	...
	while(child_element != NULL)
	{
		...
		if(entity_element!=NULL)
		{
			...
			std::string entity_mesh_str= entoty_mesh_element->GetText();
			...
			game_manager->attachEntity(entity_name_str,entity_mesh_str,entity_material_str,)
		}
...
ListArray<GameResource>* GameXML::getXMLGameResourcesByLevel(std::string requested_l)
{
	...
	while(level_element!=NULL)
	{
		...
		if(requested_level_name == level_name_str)
		{
			...
			while(mesh_element!=NULL)
			{
				...
				MeshResource* mesh_resource = new MeshResource(requested_level_name, mesh)
